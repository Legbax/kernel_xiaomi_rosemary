...
#ifdef CONFIG_KSU
	/* KernelSU: bypass seccomp for __NR_reboot with magic 0xDEADBEEF
	 * so the KSU Manager can communicate with the kernel via reboot syscall */
	if (regs->syscallno == __NR_reboot &&
	    regs->orig_x0 == 0xDEADBEEF) {
		goto skip_seccomp;
	}
#endif
