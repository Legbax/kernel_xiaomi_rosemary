...
/*
 * SUSFS compatibility functions.
 * These provide the legacy SUSFS SID-based checks using the new cached SID infrastructure.
 */
#ifdef CONFIG_KSU_SUSFS

#define KERNEL_SU_DOMAIN "su"
#define KERNEL_ZYGOTE_DOMAIN "zygote"
#define KERNEL_INIT_DOMAIN "init"
...
{
	char full_ctx[64];
	snprintf(full_ctx, sizeof(full_ctx), "u:r:%s:s0", secctx_name);
	if (security_secctx_to_secid(full_ctx, strlen(full_ctx), out_sid)) {
		pr_warn("susfs: failed to resolve SID for %s\n", full_ctx);
		*out_sid = 0;
	}
}
...
{
	struct inode_security_struct *isec = (struct inode_security_struct *)sec;
	if (!isec)
		return false;
	return isec->sid == sid2;
}
...
u32 ksu_get_devpts_sid(void)
{
	...
	int err = security_secctx_to_secid(DEVPTS_DOMAIN, strlen(DEVPTS_DOMAIN),
					   &devpts_sid);
	if (err) {
		pr_warn("ksu_get_devpts_sid failed: %d\n", err);
		return 0;
	}
	return devpts_sid;
}
